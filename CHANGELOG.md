...
+ [修复] DSH Desktop 通过宿主状态恢复上次画布开关和分屏/画布布局，避免动态端口变化后默认回到会话。
+ [调整] Agent 连接面板改为说明 DSH Desktop 自动管理本地连接，不再向普通用户展示上游 Agent 安装与卸载命令。
+ [调整] 配置导出、WebDAV 默认目录及 Agent 启动和更新提示统一使用 DSH FreeCanvas 用户入口，不再暴露上游操作命令。
+ [调整] 默认官方 AI 渠道仅保留生图和视频模型，文本与音频能力改为按需配置独立渠道。
+ [新增] 插件宿主新增默认关闭的官方媒体渠道配置，仅在单用户本机模式和有效服务端地址齐备时才允许后续启用。
+ [新增] 插件宿主新增本机凭据保险箱与同源官方 API 白名单代理，浏览器不再接触官方设备令牌。
+ [新增] 画布官方渠道改为不可编辑的代码派生项，并提供无浏览器令牌的账户连接、积分与卡密兑换入口。
