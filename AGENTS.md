...
- DSH 插件路径下的画布项目、“我的素材”、生成记录和用户配置保存在 `$DSH_HOME/storages/dsh-freecanvas` 的宿主私有本地存储，并镜像到当前浏览器 origin；独立 Web 开发模式继续使用浏览器本地存储。不要在文档中误写成已支持云同步。
- DSH 插件路径下的用户 AI API Key 保存在宿主私有本地存储与当前浏览器 origin，独立 Web 开发模式只保存在浏览器本地；AI 请求仍由前端直接请求 OpenAI 兼容接口。涉及安全说明时要写清楚。
- DSH FreeCanvas 的普通用户入口是 DSH 插件市场；README、快速开始和功能文档不要再把 Vite、Vercel、Render、Docker 或独立 Web 服务写成用户安装、启动或部署方式。源码开发命令只放在开发文档中。
