---
title: Codex App 插件
description: 在 Codex App 中安装并使用 DSH FreeCanvas 插件
---

# Codex App 插件

DSH FreeCanvas 提供了一个 Codex App 插件，用来把 Codex 和本地 Canvas Agent 连接起来。安装后，你可以直接在 Codex 中说“打开 DSH FreeCanvas”，让 Codex 打开画布、读取节点、创建节点并触发生成流程。
...
```text
请从 https://github.com/JustinQiuck/dsh-freecanvas.git 安装 DSH FreeCanvas Codex 插件。
请 clone 仓库到 ~/plugins/dsh-freecanvas，确认 plugins/infinite-canvas/.codex-plugin/plugin.json 存在，
...
确保 `~/.agents/plugins/marketplace.json` 中有 DSH FreeCanvas 条目。这里的 `path` 要指向仓库里的插件子目录：
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "local",
        "path": "./plugins/dsh-freecanvas/plugins/infinite-canvas"
      },
      "policy": {
...
如果你正在 DSH FreeCanvas 仓库中开发插件，可以直接使用仓库自带的 marketplace：
...
```text
打开 DSH FreeCanvas 新建画布并连接 Codex
```

插件会启动本地 Canvas Agent，读取连接地址和 token，然后在右侧打开本地 DSH FreeCanvas 前端并自动新建、连接画布。
